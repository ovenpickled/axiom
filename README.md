...
mv axiom-linux-x86_64 ~/.local/bin/axiom
```
...
[`.github/workflows/`](./.github/workflows/) contains two pipelines: `release.yml` builds and publishes cross-compiled binaries for all four supported targets on every version tag, and `docker.yml` builds and pushes the multi-arch Docker image to Docker Hub.
